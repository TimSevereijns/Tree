...
void VerifyTraversal(
    const std::vector<DataType>& expected, const std::vector<DataType>& actual,
    bool performSubsetComparison = false)
{
    ...
    const auto isCorrect =
        std::equal(std::begin(actual), std::end(actual), std::begin(expected), std::end(expected));
 ...
 */
template <typename DataType> void VerifyParentIsIdentical(const Tree<DataType>& tree)
{
...
TEST_CASE("Forward Pre- and Post-Order Traversal of Simple Binary Tree")
{
    ...
    SECTION("Pre-order Traversal")
    {
        ...
        std::transform(
            tree.beginPreOrder(), tree.endPreOrder(), std::back_inserter(actual),
    ...
    SECTION("Post-order Traversal")
    {
        ...
        std::vector<std::string> actual;
        std::transform(
            std::begin(tree), std::end(tree), std::back_inserter(actual),
            [](const auto& node) noexcept { return node.GetData(); });
...
TEST_CASE("STL Typedef Compliance")
{
    ...
    SECTION("Standard Algorithms and Parameter Passing by value_type")
    {
        const auto count = std::count_if(
            ...
            [](Tree<std::string>::value_type node) noexcept { return node.GetData() == "A"; });
    ...
    SECTION("Standard Algorithms and Parameter Passing by reference")
    {
        const auto count = std::count_if(
            std::begin(tree), std::end(tree),
            [](Tree<std::string>::reference node) noexcept { return node.GetData() == "C"; });
...
TEST_CASE("Leaf Iterator")
{
    ...
    SECTION("Tree<T>::beginLeaf and Tree<T>::endLeaf")
    {
        ...
        std::vector<std::string> actual;
        std::transform(
            tree.beginLeaf(), tree.endLeaf(), std::back_inserter(actual),
